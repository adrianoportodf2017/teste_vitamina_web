...
if ( ! defined( 'BOOTSPRESS_VERSION' ) ) {
	...
	define( 'BOOTSPRESS_VERSION', '0.9.2.4' );
}
...
function enviar_email_ajax() {
	if ($_SERVER["REQUEST_METHOD"] === "POST") {
	  ...
	  // Enviando o e-mail usando a função wp_mail()
	  ...
	  // Verificando se o e-mail foi enviado com sucesso
		...
		if ($state == "RJ") {
		  echo "RJ"; die;// Retorna "RJ" se o estado for Rio de Janeiro
		} else {
		  echo "success"; die;// Retorna "success" se a mensagem for enviada sem ser no Rio de Janeiro
		}
	 
	}
  }
  
  // Registrar a função no WordPress
  add_action('wp_ajax_enviar_email', 'enviar_email_ajax');
  add_action('wp_ajax_nopriv_enviar_email', 'enviar_email_ajax');